...
class _PalestineFlagScreenState extends State<PalestineFlagScreen>
      ...
      body: Container(
        ...
        decoration: BoxDecoration(
          gradient: LinearGradient(
            ...
            colors: [Colors.black, Colors.red[900]!, Colors.red[700]!],
          ),
        ),
        child: Stack(
          children: [
...
class PalestineFlagPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (outlineProgress > 0) {
      ...
      // Right line
      if (topProgress >= 1.0) {
        final rightProgress = math.min(
          1.0,
          (currentOutlineLength - width) / height,
        );
      ...
      // Bottom line
      if (topProgress >= 1.0 &&
          (currentOutlineLength - width) / height >= 1.0) {
        final bottomProgress = math.min(
          1.0,
          (currentOutlineLength - width - height) / width,
        );
      ...
      if (topProgress >= 1.0 &&
        ...
        final leftProgress = math.min(
          1.0,
          (currentOutlineLength - width * 2 - height) / height,
        );
      ...
      if (outlineProgress >= 0.7) {
        ...
        trianglePath.lineTo(
          width * 0.3 * triangleProgress,
          height * 0.5 * triangleProgress,
        );
...
class BloodSplatter extends StatelessWidget {
  ...
  const BloodSplatter({Key? key, required this.size, required this.color})
    : super(key: key);
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      width: size.width,
      height: size.height,
      child: CustomPaint(painter: BloodSplatterPainter(color: color)),
    );
  }
}
